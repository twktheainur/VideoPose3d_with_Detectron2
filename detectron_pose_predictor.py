...
if __name__ == '__main__':
    parser = ArgumentParser(description="Detectrong2 pose prediction for VideoPose3D",
                            prog="detectron_pose_predictor.py",
                            usage="%(prog)s [options]")

    parser.add_argument("--model_config", "-c", nargs=1, required=True, help="Path to the model configuration (yaml)")
    parser.add_argument("--model_weight", "-w", nargs=1, required=True, help="Path to the pickled model weights (pkl)")
    parser.add_argument("--no_cuda", action="store_true", help="Disable the use of CUDA")

    parser.add_argument("--images", action="store_true",
                        help="Set the input type to images. If set, the --input arguments should be the path" + \
                             "the a directory containing the frames of the video as individual files.")

    parser.add_argument("--input", "-i", nargs=1, required=True, help="Path to the input video. If --images is set," + \
                                                                      "should be the path to a directory that" + \
                                                                      "contains the frames of the video as images.")
    parser.add_argument("--output", "-o", nargs=1, help="Output path for the estimated poses", required=True)
    ...
    pose_predictor = init_pose_predictor(parsed_arguments.model_config, parsed_arguments.model_wights,
                                         cuda=(not parsed_arguments.no_cuda))

    # Predict poses and save the result:
    if parsed_arguments.images:
        img_generator = read_images(parsed_arguments.input)  # read images from a directory
    else:
        img_generator = read_video(parsed_arguments.input)  # or get them from a video
